...
//    final static String excelPath = "C:\\Users\\liuzy2019\\Environment.xlsx";
    final static String sheetName = "environment";

    public static void writeToExcel(TempData tempData) throws IOException {
        InputStream inputStream = new FileInputStream(excelPath);
        XSSFWorkbook workbook = new XSSFWorkbook(inputStream);
        XSSFSheet sheet = workbook.getSheet(sheetName);
        XSSFRow row;
        if(deviceExcelRow.get(tempData.getDeviceId()) == null)
        {
            deviceExcelRow.put(tempData.getDeviceId(),deviceExcelRow.size()+1);
            row = sheet.createRow(deviceExcelRow.size());
            for(int i=0;i<12;i++)
            {
                row.createCell(i);
            }
            row.getCell(0).setCellValue(tempData.getDeviceId());
        }
        row = sheet.getRow(deviceExcelRow.get(tempData.getDeviceId()));

        //ServiceID
        row.getCell(1).setCellValue("Transimission");

        //EventTime
        row.getCell(2).setCellValue(tempData.getEventTime());

        //RoomTemperture
        row.getCell(3).setCellValue(tempData.getRoomTemperature()/10.0);

        //RoomHumidity
        row.getCell(4).setCellValue(tempData.getRoomHumidity());

        //InnerTempAlarm
        row.getCell(5).setCellValue(tempData.getInnerTempAlarm());

        //ExtTempAlarm
        row.getCell(6).setCellValue(tempData.getExtTempAlarm());

        //HumitureAlarm
        row.getCell(7).setCellValue(tempData.getHumitureAlarm());

        //IMEI
        row.getCell(8).setCellValue(tempData.getImei());

        //Battery
        row.getCell(9).setCellValue(tempData.getBattery());

        //ICCID
        row.getCell(10).setCellValue(tempData.getIccId());

        //CellID
        row.getCell(11).setCellValue(tempData.getCellId());
    ...
    public static void main(String[] args) throws IOException {
        ...
        tempData.setBattery(400);
        tempData.setCellId("87652495");
        tempData.setDeviceId("alsdkjfasdlkfjasldd");
        tempData.setEventTime("20200894");
        tempData.setExtTempAlarm(0);
        tempData.setHumitureAlarm(0);
        tempData.setIccId("798768");
        tempData.setImei("182738126381");
        tempData.setInnerTempAlarm(0);
        tempData.setRoomHumidity(70);
        tempData.setRoomTemperature(294);
        tempData.setServiceId1("transimission");
        tempData.setServiceId2("data");
